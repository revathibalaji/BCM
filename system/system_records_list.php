<?
	...
	$system_records_lookup_subsection = isset ($_GET["system_records_lookup_subsection"]) ? $_GET["system_records_lookup_subsection"]:null;
	...
	if (!empty($system_records_lookup_section) AND !empty($system_records_lookup_subsection) AND !empty($system_records_lookup_item_id)) {
		$specific_query = " WHERE system_records_section=\"$system_records_lookup_section\" AND system_records_subsection=\"$system_records_lookup_subsection\" AND system_records_item_id=\"$system_records_lookup_item_id\" AND system_records_author=\"$system_records_lookup_author\" AND system_records_action=\"$system_records_lookup_action\" AND system_records_notes=\"$system_records_lookup_notes\" AND system_records_date=\"$system_records_lookup_date\"";
	} 
	 
	#actions .. edit, update or disable - YOU MUST ADJUST THIS!
	if ($system_records_lookup_action == "add_note") {
		add_system_records($system_records_lookup_section, $system_records_lookup_subsection, $system_records_lookup_item_id, $system_records_lookup_author, "Note", $system_records_notes, $system_records_date);
	}
...
$page_offset = ($page-1)*$page_limit;
	...
	if ($sort == "system_records_description" OR $sort == "system_records_section" OR $sort == "system_records_action" OR $sort == "system_records_author" OR $sort="system_records_date") {
